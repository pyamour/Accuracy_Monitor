...
class Show_Accuracy():
    ...
    def show(self):
        ...
        ylabel = "Number of Properties in Percentage (%)"
        xlabel = "Error Rate in Percentage (%)"
        ...
        df_percent_0_20 = df_percent[(df_percent.percent <= 20) & (df_percent.percent > 0)]
        sns.barplot(df_percent_0_20['percent'], 100*df_percent_0_20['percentage'], palette="BuPu_r")
        plt.title('Accuracy')
        plt.ylabel(ylabel)
        plt.xlabel(xlabel)
        ...
        """
        data = df_result
        data["Pp"] = data.apply(lambda x: round(x.Pp/1000), axis = 1)
        data["Sp"] = data.apply(lambda x: round(x.Sp/1000), axis = 1)
        h= sns.jointplot(x="Pp" , y = "Sp" ,data = data)
        h.ax_joint.set_xlabel("Prediction Price (K)")
        h.ax_joint.set_ylabel("Sold Price (K)")
        plt.show()
        """
